...
| `id`（数字） | ❌ **不下发** | ❌ 结构体里也没有（只有 `deck_id`） | **私服会多发**（实测 `fyserver` 每行多回一个数字 `id`），客户端忽略 |
...
**3. 数字 `id` 有多套来源，谁都别信。** 官服不发、客户端结构体里也没有；私服（`fyserver`）实测会**多回一个数字 `id`**（如 `1021`），客户端同样忽略。三者都印证同一件事：**卡牌身份只有 `card_type` 这一个权威键**。
:::

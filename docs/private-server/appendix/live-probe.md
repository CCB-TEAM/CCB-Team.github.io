...
::: tip 补充：私服会多回一个数字 `id`
真机客户端打到自建私服时（[附录 H](/private-server/appendix/client-capture) §10），`library` 每行**多了一个数字 `id`**（如 `1021`）——那是 `fyserver` 自己的实现字段，**官服没有**，客户端也不依赖。

所以"`id` 不存在"是**官服的结论**，不是"协议里永远不该有"：自建服务发不发都行，**但绝不能把它当主键**。
:::
